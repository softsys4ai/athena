...
for epsID in range(len(listEPS)):
    ...
    '''
    advAcc  = calAccu(advPredProb, labels)
    oriAcc  = calAccu(origPredProb, labels)
    advAccW = calAccu(advPredProb, labels, modelsExpertise)
    oriAccW = calAccu(origPredProb, labels, modelsExpertise)
    advAccMTMV = calAccu_MTMV(advPredProb, labels, modelsExpertise)
    oriAccMTMV = calAccu_MTMV(origPredProb, labels, modelsExpertise)
    ...
    '''
    with open("eps"+str(eps)+"_trans_over_orig.txt", "w") as fp:
        for tID in range(numOfTrans):
            transformType = IMG_TRANSFORMATIONS[tID]
            testAcc, aveConf, aveConfMisclassified = evaluate(orig_model, transform_images(origs, transformType), labels)
            print("[{}] Transform: {}, testAcc: {}, average confidence: {}".format(tID, transformType, testAcc, aveConf))
            fp.write(str(tID)+"\t"+transformType+"\t"+str(testAcc)+"\t"+str(aveConf)+"\n")
