...
def generate_ae(model, data, labels, attack_configs, selected_attacks,
    ...
    for id in adversaries:
        ...
        attack_args["eot"] = eot
        print('Generating AE [{}].'.format(attack_args))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--selected-pool', required=False, default="ensemble0",
    ...
    parser.add_argument('-a', '--attack-configs', required=False,
                        default='../configs/experiment/mnist/attack-wb-cnn.json',
                        help='Folder where test data stored in.')
    ...
    if args.targeted_model == 'single':
        ...
        prefix = "AE-mnist-cnn-{}".format(args.targeted_model)

        model_file = os.path.join(model_configs.get("lenet").get('dir'), model_configs.get("lenet").get("um_file"))
        ...
        pool_configs["active_wds"] = pool_configs.get(selected_pool)
    ...
    if args.benign_samples:
        data_file = os.path.join(data_configs.get("dir") + data_configs.get("zk_dir"), data_configs.get(args.benign_samples)[0])
        label_file = os.path.join(data_configs.get("dir") + data_configs.get("zk_dir"), data_configs.get(args.benign_samples)[1])
    else:
        data_file = os.path.join(data_configs.get("dir") + data_configs.get("zk_dir"), data_configs.get('bs_ratio0005')[0])
        label_file = os.path.join(data_configs.get("dir") + data_configs.get("zk_dir"), data_configs.get('bs_ratio0005')[1])
        
    print(">>> Loading data from [{}].".format(data_file))
    data_bs = np.load(data_file)
    # load the corresponding true labels
    #label_file = os.path.join(data_configs.get('dir'), data_configs.get('label_file_ratio0005'))
