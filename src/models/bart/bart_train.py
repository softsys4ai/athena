...
def _get_transformation_list(selected_combo='revisionES1_ens1', config_file=None):
    if config_file is None:
        config_file = '../../configs/experiment/cifar100/full-pool.json'

    transformation_configs = json.load(config_file)
    ensemble_size, ensemble_idx = selected_combo.split('_')
    selected_transformations = transformation_configs.get(ensemble_size).get(ensemble_idx)

    return selected_transformations, transformation_configs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--selected-combo', type=str, default='revisionES4_ens1')
    ...
    import time
    t = time.time()
    save_path = args.selected_combo + '_' + args.save
    print(f'>>> SAVE_PATH: {save_path}')
    result = train_and_eval(args.tag, args.dataroot, trans_type='clean',  test_ratio=args.cv_ratio,
